...
function setup() {
  ...
  gameOver = createSprite(178,height-480);
  //320
  gameOver.scale = 0.3;
  gameOver.addImage(gameOverImg);
  gameOver.visible = true;
  
  restart = createSprite(162,height-395);
  restart.scale = 0.7;
  ...
  f = createSprite(150,height-700,width,50);
  f.addImage(c);
  f.scale = 1.3;
  
  icons1 = createSprite(25,height-40,width,50);
...
function draw() {  
  ...
  if(gameState === PLAY){
    ...
    if((touches.length > 0 || keyDown("space")) && rocket1.y >= height-120) {
  ...
  fill("white");
  textSize(15);
  text("T H E   I N F I N I T E   R O C K E T",46,248);
  
  fill("white");
  stroke("white");
  strokeWeight(3.5);
  textSize(45);
  text("G  A  L  A  X  Y",15,220);
  ...
  strokeWeight(5);
  stroke("yellow");
  text("O",135,height-25);
  fill("lightgreen");
  strokeWeight(5);
  stroke("lightgreen");
  text("R",175,height-25);
  fill("orange");
  strokeWeight(5);
  stroke("orange");
  text("E",214,height-25);
  fill("white");
  strokeWeight(5);
  stroke("white");
  text("     :  "+ score,205,height-25);
  
  
  textSize(30);
  strokeWeight(0);
  stroke(0);
  text("   "+ points,150,height-700);
}
...
function Mount(){
 if (frameCount % 120 === 0){
   var MT = createSprite(1900,height-95,width,125);
   MT.addImage(mount3);
...
function spawnAlien(){
 if (frameCount % 350 === 0){
   var alien = createSprite(1900,height-95,width,125);
...
function spawnSP() {
  if(frameCount % 500 === 0) {           
    helmet = createSprite(1900,height-120,width,50); 
    helmet.addImage(g);
